---
title: "Managerial Creative Problem Solving and the Big Five Personality Traits"
author: Nils Myszkowski
date: '2015-06-08'
slug: []
categories:
  - Creativity
  - Management
  - Personality
tags: []
excerpt: 'In this paper, published in <a href="https://doi.org/10.1108/JMD-12-2013-0160"><em>Journal of Management Development</em></a>, we examine associations between managerial creative problem solving and the Big Five traits.'
links:
- icon: file-pdf
  icon_pack: fa
  name: PDF
  url: Publication_pdf/Myszkowski et al. - 2015 - Managerial creative problem solving and the Big Fi.pdf
---

# What it's about

In this paper, we examine managerial creative problem solving and the big five personality traits.

# Abstract

This paper examines the focal construct and reports empirical and/or psychometric evidence relevant to its interpretation. The findings contribute to the literature by clarifying measurement, theoretical implications, and practical applications in context.

# How to access the paper

Go to the journal page for this article [here](https://doi.org/10.1108/JMD-12-2013-0160).

Download the PDF [here](/Publication_pdf/Myszkowski%20et%20al.%20-%202015%20-%20Managerial%20creative%20problem%20solving%20and%20the%20Big%20Fi.pdf).
