---
title: "Individual Differences in Aesthetic Ability: The Case for an Aesthetic Quotient"
author: Nils Myszkowski
date: '2016-05-14'
journal: "Frontiers in Psychology"
doi: "10.3389/fpsyg.2016.00750"
citation_authors:
  - "Nils Myszkowski"
  - "Franck Zenasni"
pdf_url: "/Publication_pdf/Myszkowski%20and%20Zenasni%20-%202016%20-%20Individual%20differences%20in%20aesthetic%20ability%20The%20c.pdf"
slug: []
categories:
  - Aesthetics
  - Psychometrics
  - Individual differences
tags: []
excerpt: 'In this paper, published in <a href="https://doi.org/10.3389/fpsyg.2016.00750"><em>Frontiers in Psychology</em></a>, we propose and evaluate an aesthetic quotient framework for individual differences in aesthetic ability.'
links:
- icon: file-pdf
  icon_pack: fa
  name: PDF
  url: Publication_pdf/Myszkowski and Zenasni - 2016 - Individual differences in aesthetic ability The c.pdf
---

# What it's about

In this paper, we examine individual differences in aesthetic ability.

# Abstract

This paper examines the focal construct and reports empirical and/or psychometric evidence relevant to its interpretation. The findings contribute to the literature by clarifying measurement, theoretical implications, and practical applications in context.

# How to access the paper

Go to the journal page for this article [here](https://doi.org/10.3389/fpsyg.2016.00750).

Download the PDF [here](/Publication_pdf/Myszkowski%20and%20Zenasni%20-%202016%20-%20Individual%20differences%20in%20aesthetic%20ability%20The%20c.pdf).
